...
char USB_IDN_Buffer[55] = {'C','D','C',' ','T','C',' ','L','O','G','G','E','R',' ','D','E','V','I','C','E',' ','B','Y',' ','M','A','K','I','N','G',' ','D','E','V','I','C','E','S',' ','H','W',':','1','.','2','b',' ','F','W',':','1','.','5','0','\n'};
char USB_Out_Buffer[64];		//USB OUT BUFFER
unsigned char SPI_response[8];  
unsigned char SPI_lastmessage[8];
char spi_state = 0;
char length;
char LED[2] = {0,0};
int convtemperature = 0;
int convtemperature_decimal = 0;
unsigned int rem;
char com_var = 0;
char index = 0;
char index_two = 0;
unsigned char meas_1 = 0;
unsigned char meas_2 = 0;
unsigned char meas_3 = 0;
int i;
char loop = 0;
char length_first = 0;
...
void bin2temp(unsigned char binary,unsigned char binary2, unsigned char mode);
...
/** VECTOR REMAPPING ***********************************************/
    ...
    #define APP_FIRMWARE_VERSION_MINOR  5   //valid values 0-99
...
void bin2temp(unsigned char binary,unsigned char binary2, unsigned char mode){
	char neg = 0; //neg == 0 if number is positive
    convtemperature = 0;
	convtemperature_decimal = 0;

	if((binary2&0b10000000) == 0b10000000){
		neg = 1;
		if(mode==2) binary = binary^0b11111100;
		if(mode==1) binary = binary^0b11110000;
		binary2 = binary2^0b11111111;
	}

	if(mode == 1) {
		if(neg==1 && binary < 240) binary = binary+0b00010000;
		else if(neg==1) {
			binary = 0;
			binary2 += 1;
		}
		...
		if((binary&0b00010000) == 0b00010000)convtemperature_decimal += 6; //0.0625
		if((binary&0b00100000) == 0b00100000)convtemperature_decimal += 12; //0.125
		if((binary&0b01000000) == 0b01000000)convtemperature_decimal += 25;//0.25
	    if((binary&0b10000000) == 0b10000000)convtemperature_decimal += 50; //0.5
		if((binary2&0b00010000) == 0b00010000)convtemperature = convtemperature + 16;
	    if((binary2&0b00100000) == 0b00100000)convtemperature = convtemperature + 32;
	    if((binary2&0b01000000) == 0b01000000)convtemperature = convtemperature + 64;
	} else {
		mode = 16;
		if(neg==1 && binary < 252){
			binary = binary+0b00000100;
		} else if(neg==1) {
			binary = 0;
			binary2 += 1;
		}
	    ...
	    if((binary&0b00000100) == 0b100) convtemperature_decimal += 25; //0.25 
	    if((binary&0b00001000) == 0b1000)convtemperature_decimal += 50; // 0.5
		if((binary&0b00010000) == 0b00010000)convtemperature += 1;
		if((binary&0b00100000) == 0b00100000)convtemperature += 2;
	    if((binary&0b01000000) == 0b01000000)convtemperature += 4;
	    if((binary&0b10000000) == 0b10000000)convtemperature += 8;
	    if((binary2&0b00010000) == 0b00010000)convtemperature = convtemperature + 256;
	    if((binary2&0b00100000) == 0b00100000)convtemperature = convtemperature + 512;
	    if((binary2&0b01000000) == 0b01000000)convtemperature = convtemperature + 1024;
...
char ftoa(unsigned char *buf) {
        unsigned char *s,length=0,length_dec=0,dec[2];

		rem = 0;
        i = convtemperature;
 
        s = buf;
        if (i == 0 && convtemperature_decimal == 0 ){            //print 0.0 with null termination here
                *s++ = '0';
                *s++ = '.';
                *s++ = '0';
				*s++ = '0';
                *s=0;                   //null terminate the string
                ...
                if (i < 0 || convtemperature_decimal < 0) {
                        *buf++ = '-';
                        s = buf;
                        if (i < 0) i = -i;
                }
				...
				i = convtemperature_decimal;
				if (i < 0) i = -i;
        		if (i == 0 && length<4){            //print 0.0 with null termination here
                	*(buf+length+2) = '0'; //second decimal
					*(buf+length+1) = '0'; //third decimal
	                *(buf+length)='.';                   //null terminate the string
					*(s+3)=0; 
					length += 2;
        		} else if(length<4){
					rem = 0;
	                ...
	                while (i) {
	                        rem = i % 10;
	                        dec[length_dec] = rem + '0';
	                        i /= 10;
							++length_dec;
	                }
                	*(buf+length+2) = dec[0]; //second decimal
					*(buf+length+1) = dec[1]; //third decimal
	                *(buf+length)='.';                   //null terminate the string
					*(s+3)=0; 
					length += 2;
				} else{
